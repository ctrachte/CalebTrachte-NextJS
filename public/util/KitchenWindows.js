...
class DraggableWindows
{
  constructor(options)
  {
    ...
    this.hoverTracking =
      typeof options.hoverTracking !== "undefined"
        ? options.hoverTracking
        : false;
    ...
    this.backgroundHeight= this.options.backgroundHeight;
    this.backgroundWidth= this.options.backgroundWidth;
    ...
    this.developerMode =
      typeof this.options.developerMode === "boolean"
        ...
        : window.location.href.split("/").includes("localhost"); //false; - change this to false to turn all logging off.
  ...
  async login()
  {
    if (window.location.href.includes("T3")){
      this.init();
      return;
    }
    ...
    let context = this;
    let loginForm = `
      <form class="kw-bg-white kw-shadow-md kw-w-full kw-rounded kw-px-8 kw-pt-6 kw-pb-8 kw-mb-4">
        <h1 class="kw-text-center kw-bg-white kw-shadow-md kw-w-full kw-rounded kw-mb-4">${ this.title }</h1>
        <div class="kw-mb-4">
          <label class="kw-block kw-text-gray-700 kw-text-sm kw-font-bold kw-mb-2" for="username">
            Username
          </label>
          <input class="kw-shadow kw-appearance-none kw-border kw-rounded kw-w-full kw-py-2 kw-px-3 kw-text-gray-700 kw-leading-tight kw-focus:outline-none kw-focus:shadow-outline" id="username" type="text" placeholder="Username">
        </div>
        <div class="kw-mb-6">
          <label class="kw-block kw-text-gray-700 kw-text-sm kw-font-bold kw-mb-2" for="password">
            Password
          </label>
          <input class="kw-shadow kw-appearance-none kw-border kw-border-red-500 kw-rounded kw-w-full kw-py-2 kw-px-3 text-gray-700 mb-3 kw-leading-tight kw-focus:outline-none kw-focus:shadow-outline" id="password" type="password" placeholder="******************">
        </div>
        <div class="kw-flex kw-items-center kw-m-2 kw-w-full kw-justify-between">
          <button class="signInButton kw-btn-gold kw-font-bold kw-py-2 kw-px-4 kw-rounded kw-focus:outline-none kw-focus:shadow-outline" type="button">
            Sign In
          </button>
          <button style="background-color: green;" class="signUpButton kw-text-white kw-font-bold kw-py-2 kw-px-4 kw-rounded kw-focus:outline-none kw-focus:shadow-outline" type="button">
          Sign Up
          </button>
          <a class="kw-inline-block kw-align-baseline kw-font-bold kw-text-sm kw-text-blue-500 kw-hover:text-blue-800" href="#">
            Forgot Password?
          </a>
        </div>
        <p class="kw-text-center kw-text-gray-500 kw-text-xs">
          &copy;2023 Kitchen Windows. All rights reserved.
        </p>
      </form>
    `;
  ...
  async init()
  {
    ...
    this.loadingSpinner = new clsLoadingElement({
      parent: this,
  ...
  async createToolBar()
  {
    ...
    if (!this.isMobile) {
      if (this.toolBarPosition === "bottom" || this.toolBarPosition === "top")
      {
        ...
        context.toolBar.body.style.paddingRight = parseInt(this.offset) + 6 + "px";
        context.toolBar.body.style.paddingBottom = "0px";
      } else
      {
        context.toolBar.body.style.overflowX = "hidden";
        context.toolBar.body.style.overflowY = "auto";
        context.toolBar.body.style.paddingRight = "0px";
        context.toolBar.body.style.paddingBottom = parseInt(this.offset) + 6 + "px";
      }
    }
    ...
    context.toolBar.window.style.backgroundColor = `${ context.darkMode
    ...
    if (this.isMobile)
    {
      ...
      {
        if (
          e.target.id === context.toolBar.id + "-body" ||
  ...
  createSettingsWindow()
  {
    ...
    let baseTemplate = `
                <div class="kw-w-full kw-p-5 kw-text-center kw-content-center kw-items-center">
                    <h4 class="kw-pt-2 kw-fc-header-title settings-buttons">
                        <i role="button" class="pfi-profile kw-is-2 kw-m-3" title="My Profile"></i>
                        <i role="button" class="pfi-cube-logout kw-is-2 kw-m-3" title="Log out ... "></i>
                        <i role="button" class="pfi-light-bulb-lit kw-is-2 kw-mb-3 kw-ms-3 kw-me-3 kw-mt-0" title="Toggle light/dark mode ... "></i>
                        <i role="button" class="kw-inline-block pfi-thumbtack kw-is-2" title="Toggle hide/show toolbar... "></i>
                    </h4>
                    <h4 class="kw-pt-2 kw-fc-header-title">Theme:</h4>
                    <select class="kw-m-3 themeSelector" style="width:90%; color:black !important;">
                    </select>
                    <h4 class="kw-pt-2 kw-fc-header-title">Tool Bar Position:</h4>
                    <select class="kw-m-3 toolBarPositionSelector" style="width:66%; color:black !important;">
                        <option id="top">top</option>
                        <option id="bottom">bottom</option>
                        <option id="left">left</option>
                        <option id="right">right</option>
                    </select>
                    <button class="saveSettings kw-btn kw-btn-gold">Save Settings</button>
                </div>`;
    ...
    context.autoHideButton.addEventListener("click", function (e)
    {
      ...
      {
        context.autoHideButton.style.transform =
          "rotate(60deg) translateY(12px)";
      }
      if (context.autoHide)
      {
        context.setAutoHide();
        context.toolBar.body.addEventListener(
          "mouseenter",
          context.setAutoHide
        );
        context.toolBar.body.addEventListener(
      ...
      } else
      {
        context.showToolbar();
        context.setAutoHide();
        context.toolBar.body.removeEventListener(
          "mouseenter",
          context.setAutoHide
        );
        context.toolBar.body.removeEventListener(
  ...
  async getSettings()
  {
    ...
    if (this.authRequired && GetUser())
    {
      await GetSettings()
        .then((res) =>
        {
          res.SettingsID && typeof res === "object"
  ...
  setSettings(state)
  {
    ...
    this.state.SettingsID = state.SettingsID;
  ...
  createLogo(icon = false)
  {
    let context = this;
    if (typeof this.logoImage === "object") this.logoImage.remove();
    if (typeof this.divider === "object") this.divider.remove();
    if (!icon)
    {
    ...
    } else
    {
      ...
      this.logoImage.classList.add(icon, "kw-content-center", "kw-items-center", "kw-text-center");
    }
    this.logoImage.setAttribute("alt", this.title);
    this.logoImage.style = `
      height: ${ this.offset }px;
      width:  ${ this.offset }px;
      font-size: ${ icon ? "4.5rem !important" : "" };
      padding: ${ icon ? "0px" : "1rem" };
      position: fixed;
      z-index:9999999;
      opacity: 1;
      background-color: ${ this.darkMode ? this.darkModeHeaderColor : this.headerColor
      };
    `;
  ...
  positionLogo()
  {
    ...
    if (this.toolBarPosition === "left")
    {
      ...
      color: ${ this.darkMode ? this.darkModeColor : this.color };
      z-index:99999999;
      background-color: ${ this.darkMode ? this.headerColor : this.darkModeHeaderColor
        };
      `;
      this.logoImage.style.bottom = "0px";
      this.logoImage.style.left = this.toolBarVisible
        ? "0px"
        : `-${ this.toolBarWidth }`;
    ...
    } else if (this.toolBarPosition === "top")
    {
      ...
      `;
      this.logoImage.style.top = this.toolBarVisible
        ? "0px"
        : `-${ this.toolBarWidth }`;
    ...
    } else if (this.toolBarPosition === "right")
    {
      ...
      `;
      this.logoImage.style.bottom = "0px";
      this.logoImage.style.right = this.toolBarVisible
        ? "0px"
        : `-${ this.toolBarWidth }`;
      this.logoImage.style.left = "";
      this.logoImage.style.top = "";
    } else
    {
      ...
      `;
      this.logoImage.style.bottom = this.toolBarVisible
        ? "0px"
        : `-${ this.toolBarWidth }`;
      this.logoImage.style.right = "0px";
      this.logoImage.style.left = "";
    ...
    if (this.logoImage && this.icon) 
    {
      this.logoImage.style.padding = "5px";
    }
  }
  createBackground()
  {
    ...
    if (this.icon)
    {
      if (typeof this.background !== "object")
      {
        ...
        this.background.classList.add(this.icon, "kw-items-center", "kw-content-space-around");
        this.background.style.position = "fixed";
        this.background.style.height = (this.iconOffset || 320) + "px";
        this.background.style.width =  (this.iconOffset || 320) + "px";
        this.background.style.transition = "height 0.1s ease-out, width .1s ease-out";
      }
      this.background.style.top = (this.toolBar.window.getBoundingClientRect().height * .5 - (this.iconOffset/2 || 160)) + "px";
      this.background.style.left = (this.toolBar.window.getBoundingClientRect().width * .5 -  (this.iconOffset/2 || 160)) + "px";
      this.background.style.display = "flex";
      this.background.style.opacity = ".2";
    }
    else
    {
      this.background = document.createElement("IMG");
    }
    if ((this.logo || this.backgroundImage) && !this.icon)
    {
    ...
    if (this.backgroundHeight) this.background.style.height = this.backgroundHeight;
    if (this.backgroundWidth) this.background.style.width = this.backgroundWidth;
    this.toolBar.window.appendChild(this.background);
  }
  async createCustomToolBarIcons()
  {
    ...
    if (typeof this.toolBarItems !== "undefined" && this.toolBarItems.length)
    {
      await this.toolBarItems.forEach(async (item) =>
      {
        ...
        toolBarIcon.classList.add("kw-is-2", item.Icon, "kw-mx-2");
  ...
  closeToolbarWindows(event, window)
  {
    let context = this;
    if (window) {
      window.classList.add('kw-d-none');
      window.classList.remove('kw-flex');
    }
    if (event)
    {
      if (
      ...
      {
        context.appsManagerWindow.window.classList.add("kw-hidden");
        context.appsManagerWindow.window.classList.remove("kw-flex");
      }
      if (
      ...
      {
        context.infoWindow.window.classList.add("kw-hidden");
        context.infoWindow.window.classList.remove("kw-flex");
      }
      if (
      ...
      {
        context.FAQWindow.window.classList.add("kw-hidden");
        context.FAQWindow.window.classList.remove("kw-flex");
      }
      if (
      ...
      {
        context.PECPortalFeedback.window.classList.add("kw-hidden");
        context.PECPortalFeedback.window.classList.remove("kw-flex");
      }
      if (
        typeof context.settingsWindow === "object" &&
        context.settingsWindow &&
        context.settingsWindow.window !== event.target &&
        !context.settingsWindow.window.contains(event.target) &&
        context.settingsButton &&
        context.settingsButton !== event.target &&
        !context.settingsButton.contains(event.target)
      )
      {
        context.settingsWindow.window.classList.add("kw-hidden");
        context.settingsWindow.window.classList.remove("kw-flex");
      }
    } else
    {
      if (this.infoWindow)
      {
        this.infoWindow.window.classList.add("kw-hidden");
        this.infoWindow.window.classList.remove("kw-flex");
      }
      if (this.appsManagerWindow)
      {
        this.appsManagerWindow.window.classList.add("kw-hidden");
        this.appsManagerWindow.window.classList.remove("kw-flex");
      }
      if (this.FAQWindow)
      {
        this.FAQWindow.window.classList.add("kw-hidden");
        this.FAQWindow.window.classList.remove("kw-flex");
      }
      if (this.PECPortalFeedback)
      {
        this.PECPortalFeedback.window.classList.add("kw-hidden");
        this.PECPortalFeedback.window.classList.remove("kw-flex");
      }
      if (this.settingsWindow)
      {
        this.settingsWindow.window.classList.add("kw-hidden");
        this.settingsWindow.window.classList.remove("kw-flex");
      }
    }
  }
  removeWindow(id)
  {
    ...
    const apps = this.AppsManager.RunningApps.filter(x=>id!==x.id);
    //remove from memory
    let windows = this.windows.filter((x) =>
    {
      return x.id !== id;
    });
    // clear any intervals
    ...
    this.intervals.forEach((x) => {
      if (x.id === id) {
        ...
        intervals = this.intervals.filter(x => x.id !== id);
      }
    });
    // remove toolbar button from memory
    let toolBarButtons = this.toolBarButtons.filter((x) =>
    { x.id !== id + "-toolBar-Btn";
    });
    // remove toolbar button from ui
    ...
    this.AppsManager.RunningApps = apps;
    this.intervals = intervals || [];
    // add or remove scroll from body 
    this.bodyScroll();
  }
  async addWindow(options)
  {
    ...
    newWindow.draggableContent.appendChild(
      options.loadingSpinner.loadingElement
    );
  ...
  createToolBarButton(windowObj, closeWindow)
  {
    ...
    <i class="${ windowObj.options.Icon } kw-fs-2 kw-p-1" value=${ windowObj.id
      } style="color:${ windowObj.options.IconColor ||
      (context.darkMode ? context.darkModeColor : context.color)
      }"></i>
    <span class="tool-bar-button-text" value=${ windowObj.id }>${ windowObj.options.title
      }</span>
    `;
    ...
    closeIcon.classList.add("kw-is-1", "pfi-math-multiply", "kw-m-2");
  ...
  maximize(windowEl)
  {
    ...
    windowEl.snapTo(windowEl.draggableContent, "middle", "middle", true);
    this.focus(windowEl);
    this.bodyScroll();
  }
  focus(windowEl)
  ...
  {
    this.themeColors.forEach((element) =>
    {
      if (element.name === theme)
      {
        this.color = element.color;
        this.backgroundColor = element.backgroundColor;
        this.headerColor = element.headerColor;
        this.focusColor = element.focusColor;
        this.borderColor = element.borderColor;
        this.darkModeColor = element.darkModeColor;
        this.darkModeBackgroundColor = element.darkModeBackgroundColor;
        this.darkModeHeaderColor = element.darkModeHeaderColor;
        this.darkModeFocusColor = element.darkModeFocusColor;
        this.darkModeBorderColor = element.darkModeBorderColor;
      }
    });
  ...
  changeColors()
  {
    ...
    } else if (this.container.classList.contains("light") && this.darkMode)
    {
      ...
      !this.container.classList.contains("dark") &&
      !this.container.classList.contains("light")
    )
    {
      this.darkMode
        ? this.container.classList.add("dark")
    ...
    this.toolBarButtons.forEach((button) =>
    {
      ...
      button.style.backgroundColor = context.darkMode
        ? context.darkModeBackgroundColor
        : context.backgroundColor;
    });
    // polyfills for if you're not using a framework to manipulate dark/light such as tailwind
    ...
    // {
    //   x.style.color = context.darkMode ? context.darkModeColor : context.color;
    //   x.style.borderColor = context.darkMode
    //     ? context.borderColor
    //     : context.borderColor;
    //   x.style.backgroundColor = context.darkMode
    //     ? context.darkModeHeaderColor
    //     : context.backgroundColor;
    // });
    ...
    // datepicker polyfill
    this.container.querySelectorAll('.date').forEach((x) => {
      x.style.color = context.darkMode ? context.darkModeColor : context.color;
      x.style.borderColor = context.darkMode ? context.darkModeBorderColor : context.borderColor;
      x.style.backgroundColor = context.darkMode ? context.darkModeHeaderColor : context.backgroundColor;
    });
    this.container.querySelectorAll('.startBlock').forEach((x) => {
      x.style.color = context.darkMode ? context.darkModeColor : context.color;
      x.style.borderColor = context.darkMode ? context.darkModeBorderColor : context.borderColor;
      x.style.backgroundColor = context.darkMode ? context.darkModeHeaderColor : context.backgroundColor;
    });
    this.container.querySelectorAll('.endBlock').forEach((x) => {
      x.style.color = context.darkMode ? context.darkModeColor : context.color;
      x.style.borderColor = context.darkMode ? context.darkModeBorderColor : context.borderColor;
      x.style.backgroundColor = context.darkMode ? context.darkModeHeaderColor : context.backgroundColor;
    });
    ...
    if (this.backgroundImage || this.logo || this.icon)
    {
  ...
  createPopup(
    ...
    title = id,
    content = "<h3>Error retrieving popup content. Content not available.</h3>"
  )
  {
    let context = this;
    ...
    popup.body.style.backgroundColor = this.darkMode
      ...
      : this.backgroundColor;
    popup.body.style.color = this.darkMode ? this.darkModeColor : this.color;
    popup.body.classList.add("kw-bg-none", "kw-items-center");
    popup.closeButton = document.createElement("span");
    popup.closeButton.classList.add("kw-fs-2", "pfi-math-multiply");
    ...
    popup.window.classList.add("kw-hidden");
    popup.window.classList.remove("kw-flex");
  ...
  {
    if (window.classList.contains("kw-hidden"))
    {
      window.classList.remove("kw-hidden");
      window.classList.add("kw-flex");
    } else
    {
      window.classList.add("kw-hidden");
      window.classList.remove("kw-flex");
    }
  ...
  setPopupPosition(button, window)
  {
    ...
    if (context.isMobile)
    {
      window.style.position = window.style.height = "100%";
  ...
  browserWindowResize()
  {
    ...
    setTimeout(() =>
    {
      ...
      this.toggleToolBarButton();
      this.changeColors();
      this.toolBar.window.style.height = "100%";
      this.toolBar.window.style.width = "100%";
    }, 300);
  }
  adjustNavPosition()
  {
    if (!this.isMobile)
    {
      if (this.toolBarPosition === "top")
      {
        ...
        if (!this.toolBarVisible)
          this.toolBar.body.style.top = "-" + this.toolBarWidth;
      } else if (this.toolBarPosition === "bottom")
      {
        ...
        this.toolBar.body.style.bottom = "";
        if (!this.toolBarVisible)
          this.toolBar.body.style.top = window.innerHeight + "px";
      } else if (this.toolBarPosition === "left")
      {
        ...
        this.toolBar.body.style.bottom = "";
        if (!this.toolBarVisible)
          this.toolBar.body.style.left = "-" + this.toolBarWidth;
      } else if (this.toolBarPosition === "right")
      {
        this.toolBar.body.style.width = this.toolBarWidth;
        this.toolBar.body.style.height = "100%";
        ...
        this.toolBar.body.style.bottom = "";
        if (!this.toolBarVisible)
          this.toolBar.body.style.right = "-" + this.toolBarWidth;
      }
    } else
    {
      if (this.expanded)
    ...
    {
      this.toolBar.body.classList.add("kw-content-center");
    }
    ...
    if (!this.isMobile)
    {
      if (this.toolBarPosition === "bottom" || this.toolBarPosition === "top")
      {
        this.toolBar.body.style.paddingRight = parseInt(this.offset) + 6 + "px";
        this.toolBar.body.style.paddingBottom = "0px";
      } else if (
        this.toolBarPosition === "left" ||
        this.toolBarPosition === "right"
      )
      {
        this.toolBar.body.style.paddingRight = "0px";
        this.toolBar.body.style.paddingBottom = parseInt(this.offset) + 6 + "px";
      }
    }
    this.state.toolBarPosition = this.toolBarPosition;
    this.toggleToolBarButton();
  ...
  toggleToolBarButton()
  {
    ...
    if (this.toolBarPosition === "top")
    {
      this.hideToolBarButton.classList.add(
        "kw-is-1",
        "pfi-shape-triangle-down-solid"
      );
      this.hideToolBarButton.style.left =
        window.innerWidth / 2 -
        this.hideToolBarButton.getBoundingClientRect().width / 2 +
        "px";
    ...
    } else if (this.toolBarPosition === "left")
    {
      this.hideToolBarButton.classList.add(
        "kw-is-1",
        "pfi-shape-triangle-right-solid"
      );
      this.hideToolBarButton.style.top =
        window.innerHeight / 2 -
        this.hideToolBarButton.getBoundingClientRect().height / 2 +
        "px";
  ...
  expandToolBar()
  {
    ...
    if (!this.isMobile)
    {
      if (this.toolBarPosition === "left" || this.toolBarPosition === "right")
      {
        ...
        this.staticButtons.classList.add("kw-flex-row");
        this.staticButtons.classList.remove("kw-flex-col");
        setTimeout(function ()
        {
    ...
    } else
    {
      ...
      this.toolBar.body.style.paddingRight = "";
      this.toolBar.body.style.paddingBottom = "";
      setTimeout(function ()
      {
        context.positionWindows();
      }, 100);
  ...
  collapseToolbar()
  {
    ...
    {
      this.expanded = false;
      this.staticButtons.classList.remove("kw-flex-row");
      this.staticButtons.classList.add("kw-flex-col");
      document
        .querySelectorAll(".static-toolbar-button-text")
  ...
  hideToolBarButtons()
  {
    this.toolBarButtons.forEach((button) =>
    {
      button.style.flexDirection = this.flexDirection;
  ...
  checkScreenSize(splitDirection = "x", orientation)
  {
  ...
  removeInjection(filename, filetype)
  {
    ...
    for (let i = Scripts.length; i >= 0; i--)
    {
      ...
      if (
        Scripts[ i ] &&
        Scripts[ i ].getAttribute(Attribute)
      )
      {
        if (currentScript.getAttribute(Attribute).includes(filename))
        {
  ...
  announce(videoURL, imageURL, imageAlt = "Announcement", once = true)
  {
    ...
    playIcon.classList.add("kw-btn", "pfi-media-play", "kw-is-4", "kw-text-danger");
    ...
    if (typeof imageURL === "string")
    {
      ...
      media.onload = function ()
      {
        ...
        {
          let video = `
                        <video autoplay controls src="${ videoURL }" alt="${ imageAlt }" title="${ imageAlt }" class="kw-align-middle" type="video/mp4">
  ...
  modal(content, userCanClose = true)
  {
    ...
    if (this.modalWindow && this.developerMode)
    {
      console.error(
    ...
    this.modalWindow.window.style.padding = "2rem";
  }
  ...
  )
  {
    ...
    let closeConfirm = `
                ...
                class="kw-text-center kw-place-content-center">
                    <h3>${ heading || "Confirm:" }</h3>
                    <div class="kw-text-center kw-align-bottom">
                        <button class="acceptModalBtn kw-bg-yellow-500 kw-hover:bg-yellow-700 kw-py-2 kw-px-4 kw-rounded-md">Yes</button><button class="kw-m-3 dismissModalBtn kw-bg-gray-500 kw-py-2 kw-px-4 kw-rounded-md">No</button>
                    </div>
                </div>
                `;
  ...
  warn(heading, complete = function () { })
  {
    ...
    let closeConfirm = `
                <div class="kw-p-4 kw-w-full kw-h-full kw-text-center">
                    <h4 class="kw-text-center">${ heading }</h4>
                    <div class="kw-text-center kw-align-bottom">
                        <button style="color:${ context.darkMode
        ? context.darkModeColor
        : context.color
      };
                         background-color:${ context.darkMode
        ? context.darkModeHeaderColor
        : context.headerColor
      };
                         border: solid 1px ${ context.borderColor };"
                         "
                         class="kw-text-center acceptModalBtn kw-m-3 kw-px-4 kw-py-2 kw-rounded-md">OK</button>
                    </div>
                </div>
                `;
    closeModalContent.innerHTML = closeConfirm;
  ...
  callout(text, color = "green", id)
  {
    ...
    if (!id)
    {
      ...
      {
        let calloutTemplate = `
          <div class="kw-scrolling-container">
          </div>
        `;
      ...
      if (color === "text-danger" || color === "danger") 
      {
        CalloutText.classList.add('bg-danger', 'bg-red-700');
      }
      else if (color === "text-warning" || color === "warning") 
      {
        CalloutText.classList.add('bg-warning', 'bg-yellow-700');
      }
      else if (color === "text-success" || color === "success") {
      CalloutText.classList.add('bg-success', 'bg-green-700');
      }
      else {
      ...
      this.queue.addTimer(function ()
      {
        context.calloutMessageContainer
          .querySelector(".kw-scrolling-container")
  ...
  hoverWindow(e)
  {
    ...
    this.windows.forEach((x) =>
    {
      ...
      {
        if (this.hoveredWindow && this.hoveredWindow.id !== x.id)
        {
          this.hoveredWindow = x;
          this.AppsManager.RunningApps.filter((app) =>
          {
            if (app.id === this.hoveredWindow.id) this.hoveredApp = app;
          });
        } else if (!this.hoveredWindow)
        {
          this.hoveredWindow = x;
          this.AppsManager.RunningApps.filter((app) =>
          {
        ...
        await Promise.all(
          result.map(async (app) =>
          {
            if (!app.URL && app.Active)
            {
              ...
              await this.AppsManager.start(app).then(
                async (res) =>
                {
                  ...
                  {
                    this.removeWindow(res.initOptions.id);
                    console.log(
                      "%cExiting  " +
                      app.AppTitle +
                      " . . . total loading time: " +
