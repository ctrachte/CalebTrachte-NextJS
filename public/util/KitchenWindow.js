...
class DraggableWindow
{
  constructor(options)
  {
    ...
    this.parent = options.parent; // reference to GlobalWindowManager
    ...
    this.offset = parseInt(sanitized); // toolbarWidth converted to integer value
    ...
    this.window.classList.add("kw-noscroll");
    ...
    this.maximize =
      typeof options.maximize === "undefined" || options.maximize === null
        ? this.close
        : options.maximize;
    // callouts - can be enabled/disabled in options, used to display message internally within window.
    ...
    )
    {
      ...
      this.callouts.containerElement
        ...
        .classList.add("kw-pt-4", "kw-text-center");
    ...
    if (this.close)
    {
      ...
      this.closeButton.innerHTML = `
                <b type="button" class="closeWindowButton kw-is-1 kw-ms-1 kw-mt-0 pfi-math-multiply" aria-label="Close"></b>
            `;
      if (!this.hasManager)
      {
    ...
    if (this.maximize && this.hasManager)
    {
      ...
      this.maximizeButton.innerHTML = `
                <b type="button" class="kw-is-1 kw-mt-1 pfi-maximize kw-fa-solid maximizeWindowButton" aria-label="Maximize"></b>
            `;
    ...
    if (this.minimize && this.hasManager)
    {
      ...
      this.minimizeButton.innerHTML = `
                <b type="button" class="kw-is-1 pfi-minimize kw-fa-solid minimizeWindowButton" aria-label="Minimize"></b>
            `;
    ...
    this.draggableContent.style = `
        ...
        : this.parent.color
            ...
            background-color: ${ this.parent.darkMode
        ...
        : this.height
          ...
          : "auto"
            ...
            overflow: auto;
            border: 1px solid ${ this.parent.darkMode
    ...
    {
      let borderColor = this.parent.darkMode
        ? this.parent.darkModeBorderColor
        : this.parent.borderColor;
      this.borderColor = this.options.IconColor
      ...
      this.header.style = `
          ...
          : this.parent.Color
            ...
            left: ${ this.left };
            top: ${ this.top };
            border-color: ${ borderColor } ${ borderColor } ${ this.borderColor
        } ${ borderColor };
            border-style: solid;
    ...
    this.scrollButton.innerHTML = `<button class="kw-m-0 kw-p-0 pfi-arrow-down kw-is-1" id="scrollButton-${ this.id }" title="go to bottom or bottom of window"></button>`;
    ...
    if (this.footer)
    {
      this.footer.style = `
          ...
          : this.parent.Color
            ...
            border-color: ${ this.parent.darkMode
          ? this.parent.darkModeBackgroundColor
          : this.parent.backgroundColor
        };
      ...
      this.footer.style.top = `${ this.draggableContent.getBoundingClientRect().height +
  ...
  dragElement(el)
  {
    ...
    function elementDrag(e)
    {
      ...
      if (context.footer)
      {
        ...
        context.footer.style.width =
          parseInt(el.style.width.replace("px", "")) + "px";
        context.footer.style.top = `${ parseInt(el.style.height.replace("px", "")) +
          parseInt(el.style.top.replace("px", "")) -
          context.footer.getBoundingClientRect().height -
          1
          }px`;
      }
      if (context.snapping)
  ...
  snapTo(el, x, y, override = false)
  {
    ...
    } else
    {
      ...
      if (context.footer)
      {
        ...
        setTimeout(function ()
        {
          context.footer.style.top = `${ parseInt(context.window.getBoundingClientRect().height) +
    ...
    {
      context.footer.style.width =
        parseInt(el.style.width.replace("px", "")) - 2 + "px";
      context.footer.style.left = el.style.left;
      context.footer.style.top = `${ parseInt(el.style.height.replace("px", "")) +
        parseInt(el.style.top.replace("px", "")) -
        context.footer.getBoundingClientRect().height -
  ...
  showShadow(x, y)
  {
    // depending on orientation and locked, dont show max shadow
    if (this.isMobile) {
      if (window.innerHeight > window.innerWidth) {
        if (!this.locked && y === "middle") {
          this.hideShadows();
          return;
        } 
      } else {
        if (!this.locked && x === "middle") {
          this.hideShadows();
          return;
        }
      }
    }
    if (!this.locked && x === "middle" && y === "middle") {
      this.hideShadows();
      return;
    }
